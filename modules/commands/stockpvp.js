...
module.exports = {
    config: {
        name: "pvbstock",
        description: "PVBR auto-stock every 1,6,11... +20s delay with rare seed alerts",
    ...
    letStart: async function({ message, args }) {
        ...
        if(option==="on"){
            if(gcData.enabled) return message.reply("⚠️ Auto-stock already active!");
            gcData.enabled = true;
            await setData(`pvbstock/${channelId}`, gcData);
            this.startAutoStock(message.channel);
            return message.reply("✅ PVBR Auto-stock enabled. Runs every 1,6,11,... +20s delay.");
        }

        if(option==="off"){
            gcData.enabled = false;
            await setData(`pvbstock/${channelId}`, gcData);
            this.stopAutoStock(message.channel);
            return message.reply("❌ PVBR Auto-stock disabled.");
        }

        if(option==="check"){
            return message.reply(`📊 PVBR Auto-stock: ${gcData.enabled ? "ON ✅" : "OFF ❌"}`);
        }

        return message.reply("⚙️ Usage: /pvbstock on|off|check");
    },

    // Resume auto-stock on bot restart
        ...
        for(const channelId in allGCs){
            if(allGCs[channelId].enabled){
                ...
                if(channel) {
                    ...
                    channel.send("♻️ Bot restarted — PVBR Auto-stock resumed.");
                }
            }
        }
    }
