...
module.exports = {
  ...
  async sendStock(channel) {
    ...
    if (seeds.some((i) => this.getRarity(i.name) === "godly" && (i.currentStock ?? 0) > 0)) pingRoles.push(RARITY_ROLES.godly);
    if (seeds.some((i) => this.getRarity(i.name) === "secret" && (i.currentStock ?? 0) > 0)) pingRoles.push(RARITY_ROLES.secret);
